...
* Short = Single-character shorthand for Argument. **Used for Arguments that will most commonly be set by users.**
...
| --nyquist_limit | --nyq_lim | | Limit minimum orbital period by pseudo-Nyquist limit. | true | Minimum time spacing is used to define pseudo-Nyquist limit. |
...
| --generalized_lomb_scargle_minimum_observations | --gls_min_obs | | Minimum number of observations needed to run periodogram. | 4 |
| --generalized_lomb_scargle_frequencies | --gls_freqs | | Number of periodogram trial frequencies. | 1,000,000 | 
| --generalized_lomb_scargle_trust_power | --gls_tp | | Lowest normalized peak power needed to constrain Genetic Algorithm. | 0.50 |
| --generalized_lomb_scargle_trust_logfap | --gls_tlogfap | | Highest log false-alarm probability for acceptable periodogram period. | -4.0 |
| --generalized_lomb_scargle_trust_fraction | --gls_tf | | Fraction of adopted Lomb-Scargle period the Genetic Algorithm searches around. | 0.05 |
...
| --hooke_jeeves_shrink_fraction | --shrk_fact  | | Shriking fraction between Hooke-Jeeves exploratory and pattern moves. | 0.95 | |
...
| --metropolis_hastings_chains | --chns | | Number of sampling chains used in Metropolis-Hastings algorithm. | 100 | Set to 0 to turn off posterior sampling. |
| --metropolis_hastings_chain_burn_in | --bi | | Number of burn-in samples used in Metropolis-Hastings algorithm. | 1000 | |
| --metropolis_hastings_chain_samples | --chn_smpls | | Number of samples after burn-in for each chain used in Metropolis-Hastings algorithm. | 10,000 | Set to 0 to turn off posterior sampling. |
